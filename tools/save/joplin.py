...
def create_note(parent_id: str, title: str, body: str):
    data = {"parent_id": parent_id, "title": title, "body_html": body}
    response = requests.post(
        f"{JOPLIN_API_URL}/notes?token={os.getenv('JOPLIN_TOKEN')}", json=data)
...
def list_folders():
    response = requests.get(
        f"{JOPLIN_API_URL}/folders?token={os.getenv('JOPLIN_TOKEN')}")
    if response.status_code == 200:
...
def main():
    ...
    group.add_argument("-l", "--list", action="store_true",
                       help="List folders with their IDs")

    parser.add_argument("-p", "--parent-id", type=str,
    ...
    if args.list:
        ...
        title = args.title or datetime.datetime.now().strftime(
            "Imported - %y-%m-%d_%H-%M-%S")
